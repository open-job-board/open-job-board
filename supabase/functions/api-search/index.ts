...
//
// Proxies POST requests to PostgREST /rest/v1/rpc/search_jobs, forwarding
...
Deno.serve(async (req: Request) => {
  ...
  const tracer = createTracer("api-search");
  ...
  // Health check
  if (req.method === "GET") {
    return json({ endpoint: "api-search", status: "ok" }, 200);
  }
  ...
  const span = tracer.startSpan("POST /api/search", {
    "http.method": "POST",
  ...
  try {
    ...
    const postgrestUrl = `${SUPABASE_URL}/rest/v1/rpc/search_jobs`;
